...
if(!db.users)db.users={};if(!db.claims)db.claims=[];if(!db.wallet)db.wallet=[];if(!db.bets)db.bets=[];if(!db.games)db.games=[];if(!db.coinRequests)db.coinRequests=[];function load(){try{return JSON.parse(fs.readFileSync(DB_PATH,'utf8'))}catch{return {settings:{siteName:'MHADEV BOOK',supportTelegram:'',supportWhatsapp:''},users:{},claims:[],wallet:[],bets:[],games:[],coinRequests:[]}}}function save(){fs.writeFileSync(DB_PATH+'.tmp',JSON.stringify(db,null,2));fs.renameSync(DB_PATH+'.tmp',DB_PATH)}
