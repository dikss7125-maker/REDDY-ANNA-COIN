...
// Global feature locks controlled from Master Panel.
// These are deliberately separate from the user's coin/wallet data.
if(!db.master.locks) db.master.locks={casino:true,matches:true,odds:true};
app.get('/api/public/locks',(req,res)=>{
  const l=db.master?.locks||{casino:true,matches:true,odds:true};
  res.json({ok:true,locks:{casino:l.casino!==false,matches:l.matches!==false,odds:l.odds!==false}});
});
