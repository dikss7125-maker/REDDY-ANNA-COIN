...
const AUTH_SECRET = process.env.AUTH_SECRET || "replace-this-secret";
...
function setAuthCookie(res, type, id) {
  res.cookie(type, sign({type,id,exp:Date.now()+1000*60*60*24*30}), {
    httpOnly:true, sameSite:"lax", secure:process.env.NODE_ENV==="production", path:"/"
  });
}
function clearAuthCookie(res, type) {
  res.clearCookie(type, {httpOnly:true, sameSite:"lax", secure:process.env.NODE_ENV==="production", path:"/"});
}
...
app.post("/api/logout",(req,res)=>{clearAuthCookie(res,"rc_player");res.json({ok:true})});
...
app.post("/api/claim",requirePlayer,(req,res)=>{
  ...
  const result = db.transaction(()=>{
    ...
    const n = now();
    const inserted = db.prepare("INSERT INTO claims(bonus_id,player_id,coins,claimed_at) VALUES(?,?,?,?)").run(bonus.id,req.player.id,bonus.coins,n);
...
app.post("/api/withdraw",requirePlayer,(req,res)=>{
  ...
  const result = db.transaction(()=>{
    ...
    const ins = db.prepare(`
      INSERT INTO withdrawals(player_id,site_id,member_id,coins,status,created_at)
      VALUES(?,?,?,?, 'PENDING',?)
    `).run(req.player.id,siteId,memberId,coins,now());
...
app.post("/api/admin/logout",requireAdmin,(req,res)=>{clearAuthCookie(res,"rc_admin");res.json({ok:true})});
...
app.get("/api/admin/data",requireAdmin,(req,res)=>{
  ...
  const claims = db.prepare(`
    SELECT c.id,b.code,c.coins,p.username,c.claimed_at
    FROM claims c JOIN bonuses b ON b.id=c.bonus_id JOIN players p ON p.id=c.player_id
    ORDER BY c.id DESC
  `).all();
  ...
  const withdrawals = db.prepare(`
    SELECT w.*,p.username,s.name AS site_name
    FROM withdrawals w JOIN players p ON p.id=w.player_id JOIN sites s ON s.id=w.site_id
    ORDER BY w.id DESC
  `).all();
  res.json({settings:{
    siteName:getSetting("site_name","REDDY COIN"),
...
app.post("/api/admin/settings",requireAdmin,(req,res)=>{
  const allowed = {
    siteName:"site_name",welcome:"welcome",supportNumber:"support_number",
    telegramUrl:"telegram_url",websiteUrl:"website_url",minimumWithdrawal:"withdrawal_min_coins"
  };
...
app.post("/api/admin/logo",requireAdmin,upload.single("image"),(req,res)=>{
  ...
  setSetting("site_logo",`/uploads/${req.file.filename}`);res.json({ok:true});
});
app.post("/api/admin/bonus",requireAdmin,(req,res)=>{
  const code=String(req.body.code||"").trim().toUpperCase(), coins=Number(req.body.coins);
  if(!code || !Number.isInteger(coins) || coins<1) return res.status(400).json({error:"Enter a valid code and coin amount."});
  try{
    db.prepare("INSERT INTO bonuses(code,coins,active,created_at) VALUES(?,?,1,?)").run(code,coins,now());
    res.json({ok:true});
  }catch{res.status(409).json({error:"This bonus code already exists."})}
});
app.post("/api/admin/bonus/:id/toggle",requireAdmin,(req,res)=>{
  db.prepare("UPDATE bonuses SET active=CASE active WHEN 1 THEN 0 ELSE 1 END WHERE id=?").run(Number(req.params.id));res.json({ok:true});
});
app.post("/api/admin/site",requireAdmin,(req,res)=>{
  const name=String(req.body.name||"").trim(),url=String(req.body.url||"").trim();
  if(!name||!/^https?:\/\//i.test(url)) return res.status(400).json({error:"Enter a site name and valid https/http URL."});
...
app.post("/api/admin/site/:id/toggle",requireAdmin,(req,res)=>{
  db.prepare("UPDATE sites SET active=CASE active WHEN 1 THEN 0 ELSE 1 END WHERE id=?").run(Number(req.params.id));res.json({ok:true});
});
...
app.post("/api/admin/offer/:id/toggle",requireAdmin,(req,res)=>{
  db.prepare("UPDATE offers SET active=CASE active WHEN 1 THEN 0 ELSE 1 END WHERE id=?").run(Number(req.params.id));res.json({ok:true});
});
